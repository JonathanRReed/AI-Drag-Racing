...
export function buildLlmsFullText(): string {
  const lines: string[] = [
    ...
    '',
    'Linking in: /?model=<openrouter id>&provider=openrouter prefills model search. After supplying an OpenRouter key, select the model checkbox to add it to the race.',
    '',
    '## Definitions',
    '',
